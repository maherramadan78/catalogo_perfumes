(() => {
    ...
    function configLista() {
        ...
        return firebaseConfig.apiKey && !String(firebaseConfig.apiKey).includes('PEGA_AQUI');
    }

    function iniciarFirebase() {
        if (!configLista()) {
            mostrarEstado('Configura Firebase en firebase-config.js antes de usar el admin.', 'warning');
            return false;
        }
    ...
    function correoActual() {
        return String((auth && auth.currentUser && auth.currentUser.email) || '').toLowerCase();
    }

    function actualizarCorreos(user = null) {
        if (adminCorreoPermitido) adminCorreoPermitido.textContent = adminEmail || 'no configurado';
        if (adminCorreoActivo) {
            adminCorreoActivo.textContent = String((user && user.email) || correoActual() || 'sin sesion').toLowerCase();
        }
    }
    ...
    function esErrorPermisos(err) {
        ...
        return texto.includes('permission')
            ...
            || texto.includes('storage/unauthorized');
    }
    ...
    function mensajeOperacion(err, accion) {
        ...
        return `No se pudo ${accion}. Firebase te reconoce como ${actual}. El admin permitido es ${permitido}. Si no son iguales, sal y entra con el correo admin exacto. Si son iguales, vuelve a desplegar firestore.rules y storage.rules.`;
    }
    ...
    async function subirFotoSiExiste(idProducto) {
        ...
        const ref = storage.ref(`productos/${idProducto}-${Date.now()}.${extension}`);
        await ref.put(archivo, { contentType: archivo.type });
    ...
    async function guardarProducto(evento) {
        ...
        try {
            ...
            const docRef = idActual ? db.collection('productos_admin').doc(idActual) : db.collection('productos_admin').doc();
            const id = docRef.id;
            const datos = datosFormulario();
            const fotoUrl = await subirFotoSiExiste(id);
            const actualizado = firebase.firestore.FieldValue.serverTimestamp();
            const completo = { ...datos, fotoUrl, actualizado };

            const batch = db.batch();
            batch.set(db.collection('productos_admin').doc(id), completo, { merge: true });
            batch.set(db.collection('catalogo_menudeo').doc(id), {
                nombre: datos.nombre,
                marca: datos.marca,
            ...
            batch.set(db.collection('catalogo_mayoreo').doc(id), {
                nombre: datos.nombre,
                marca: datos.marca,
                descripcion: datos.descripcion,
                categoria: datos.categoria,
                precio_mayoreo: datos.precio_mayoreo,
                minimo_mayoreo: datos.minimo_mayoreo,
                fotoUrl,
                stock: datos.stock,
                destacado: datos.destacado,
                activo_mayoreo: datos.activo_mayoreo,
                actualizado
            }, { merge: true });
            await batch.commit();
    ...
    async function cargarProductos() {
        ...
        const snap = await db.collection('productos_admin').get();
        productos = snap.docs
            .map(doc => ({ id: doc.id, ...doc.data() }))
            .sort((a, b) => Number(b.destacado) - Number(a.destacado) || String(a.nombre).localeCompare(String(b.nombre)));

        if (!productos.length) {
            listaAdmin.innerHTML = '';
            mostrarEstado('No hay productos en Firebase. Puedes importar datos iniciales.', 'info');
            return;
        }
    ...
    function renderProductoAdmin(p) {
        ...
        const foto = p.fotoUrl || 'assets/sin-foto.svg';
    ...
    async function cambiarVisibilidad(id, visible) {
        ...
        const batch = db.batch();
        batch.set(db.collection('productos_admin').doc(id), { activo: nuevo, activo_mayoreo: nuevo }, { merge: true });
        batch.set(db.collection('catalogo_menudeo').doc(id), { activo: nuevo }, { merge: true });
        batch.set(db.collection('catalogo_mayoreo').doc(id), { activo_mayoreo: nuevo }, { merge: true });
        await batch.commit();
        await cargarProductos();
    }
    ...
    async function importarSeed() {
        try {
            ...
            const batch = db.batch();
            seed.forEach(item => {
                const id = item.id || db.collection('productos_admin').doc().id;
                const datos = {
                    nombre: item.nombre || '',
                    marca: item.marca || '',
                    ...
                    fotoUrl: item.fotoUrl || 'assets/sin-foto.svg',
                ...
                batch.set(db.collection('productos_admin').doc(id), datos, { merge: true });
                batch.set(db.collection('catalogo_menudeo').doc(id), {
                    nombre: datos.nombre,
                    marca: datos.marca,
                    descripcion: datos.descripcion,
            ...
            mostrarEstado(err.message || 'No se pudo importar.', 'danger');
        }
    }
    ...
    formLogin?.addEventListener('submit', async evento => {
        ...
        if (!adminEmail || email !== adminEmail) {
            ...
            try {
                ...
                // Nothing to do; the user is not allowed either way.
            }
            return;
        }
    ...
    btnSalir?.addEventListener('click', () => auth.signOut());

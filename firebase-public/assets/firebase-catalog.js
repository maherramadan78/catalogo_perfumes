(() => {
    ...
    const carritoKey = `carrito_firebase_${tipo}_v1`;
    ...
    function iniciarFirebase() {
        if (!configLista()) {
            mostrarEstado('Configura Firebase en firebase-config.js antes de usar este catalogo.', 'warning');
            return false;
        }
    ...
    function productoDesdeDoc(doc) {
        ...
        return {
            ...
            precio,
            foto: data.fotoUrl || 'assets/sin-foto.svg',
    ...
    async function cargarCatalogo() {
        ...
        const coleccion = esMayoreo ? 'catalogo_mayoreo' : 'catalogo_menudeo';
        const campoActivo = esMayoreo ? 'activo_mayoreo' : 'activo';
        const snap = await db.collection(coleccion).where(campoActivo, '==', true).get();
        const productos = snap.docs
            .map(productoDesdeDoc)
            .filter(p => p.precio > 0)
    ...
    btnWhatsApp?.addEventListener('click', () => {
        ...
        if (!numero || numero === '5210000000000') {
            alert('Configura tu numero de WhatsApp en firebase-config.js.');
            return;
        }
    ...
    async function iniciarMayoreo() {
        ...
        form?.addEventListener('submit', async evento => {
            ...
            try {
                await auth.signInWithEmailAndPassword(
                    document.getElementById('emailMayoreo').value.trim(),
                    document.getElementById('passwordMayoreo').value
                );
            } catch (err) {
        ...
        auth.onAuthStateChanged(async user => {
            ...
            try {
                ...
                mostrarEstado('No se pudo cargar mayoreo. Revisa las reglas y que el usuario tenga acceso.', 'danger');
            }
        });
    }
    ...
    if (esMayoreo) {
        ...
        cargarCatalogo().catch(() => {
            mostrarEstado('No se pudo cargar el catalogo. Revisa Firebase y las reglas.', 'danger');
        });
    }
})();
